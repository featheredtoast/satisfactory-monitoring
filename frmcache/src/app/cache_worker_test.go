...
func TestCache(t *testing.T) {
	...
	defer frm.Close()
	mock.ExpectBegin()
	mock.ExpectExec(`^insert into cache \(metric,frm_data\)(.+)`).WithArgs("worldInv", `{"Name":"Quickwire","Count":500}`).WillReturnResult(sql.NewResult(1,1))
	mock.ExpectExec(`^insert into cache \(metric,frm_data\)(.+)`).WithArgs("worldInv", `{"Name":"Wire","Count":500}`).WillReturnResult(sql.NewResult(1,1))
	mock.ExpectCommit()
...
func TestCacheHistory(t *testing.T) {
	...
	defer frm.Close()
	mock.ExpectBegin()
	mock.ExpectExec(`^insert into cache \(metric,frm_data\)(.+)`).WithArgs("drone", `{"ID":"0","VehicleType":"Drone"}`).WillReturnResult(sql.NewResult(1,1))
	mock.ExpectExec(`^insert into cache \(metric,frm_data\)(.+)`).WithArgs("drone", `{"ID":"1","VehicleType":"Drone"}`).WillReturnResult(sql.NewResult(1,1))
	mock.ExpectCommit()
	mock.ExpectBegin()
